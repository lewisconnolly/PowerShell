...
function Set-VMMemoryHotAddOption {
    Param (        
    ...
    $originalVMList = $VMs
    $VMs = $originalVMList | ? {$_.ExtensionData.Config.MemoryHotAddEnabled -ne $Enabled}

    if($VMs){
    
        "VMs requiring memory hot-add change: $( ($VMs | sort Name).Name -join ', ' )" | Write-Host        
        ...
        # Disable or enable memory hot-add
        
        if($Enabled){ "Enabling memory hot-add" | Write-Host }else{ "Disabling memory hot-add" | Write-Host }
        
        $vmConfigSpec = New-Object VMware.Vim.VirtualMachineConfigSpec    
        $vmConfigSpec.MemoryHotAddEnabled = $Enabled    
        (Get-View $VMs).ReconfigVM($vmConfigSpec)
                    
        # Find VMs where memory hot-add change failed

        if((Get-VM -Id $VMs.Id).ExtensionData.Config.MemoryHotAddEnabled -contains (!$Enabled)){
            
            "Memory hot-add failed to change on $( (Get-VM -Id $VMs.Id | ? {$_.ExtensionData.Config.MemoryHotAddEnabled -ne $Enabled}).Name -join ', ' )" | Write-Host
        }
    ...
    }else{ "No memory hot-add options need to be changed" | Write-Host }
        
    # Output all provided VMs and their memory hot-add configuration
