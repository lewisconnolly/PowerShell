<#
.SYNOPSIS
    Create replicas of Pure Storage vVol VMs that are members of replication enabled FlashArray Protection Groups.  
.DESCRIPTION
...
.PARAMETER SPBMReplicationGroups
    The object of a Storage Policy-Based Management Replication Group that represents a FlashArray Protection Group. Returned by Get-SpbmReplicationGroup Cmdlet.
...
.PARAMETER DestinationCluster
    The destination cluster where replica VMs are to be registered. Must be connected to the target FlashArray's vVol datastore.
.PARAMETER DestinationFolder
    The destination folder where the replica VMs are to be registered. Must be available in the same datacenter as the target FlashArray's vVol datastore.
.PARAMETER FlashArrayCredential
    The credential object for the source and target Pure Storage FlashArrays. Username should not contain domain name prefix, e.g. "lewisc" not "zonalconnect\lewisc".
...
.PARAMETER RemoveSourceVMs
    Shut down the source VMs and remove from inventory after replica VMs have been created. Cannot be used with -RemoveSourceVMsPermanently (see NOTES section).
.EXAMPLE    
    #
...
#>
function New-PureStoragevVolVMReplicas {
    ...
    DynamicParam {                  
        ...
        if(!$RemoveSourceVMs){                                               
            ...
            $permRemoveAlias = New-Object System.Management.Automation.AliasAttribute 'Remove'
    ...
    Begin {                    
        try {
            ...
            'VMware.Vim', 'VMware.VimAutomation.Core', 'VMware.VimAutomation.Storage', 'PureStoragePowerShellSDK' | % {
    ...
    Process {            
        try {
            ...
            # Get VMs or replication groups to be processed in a way that works with pipeline or without
            ...
            # End block runs if return used in Process block so $beginExit is used instead to exit from function if error encountered in Process block
    ...
    End {
        try {
            ...
            # Exit function if error or user chooses to exit in Begin or Process block
            ...
            "$( (Get-Date).ToString() ) Getting source and target replication groups for:`n`n" + ($vmsToProcess.Name -join "`n") + "`n" | Write-Verbose
            ...
            # If no VMs target repl groups found then error, else warn where failed to get source group, target group or target group already failed over
            ...
            } else { throw "`nUnable to get target replication groups for any VMs`n" }
            ...
            $vmsAndReplGroups | Group SrcReplGroup | % {
                ...
                if($SyncProtectionGroups){
                    ...
                    $newSnap = New-PfaProtectionGroupSnapshot -Array $sourceFlashArray -ProtectionGroups ($srcGroup.Name -split ':')[1] -ReplicateNow -ApplyRetention
                ...
                # For spacing printed output correctly when not using -Verbose
                ...
                if(!$replicaVmPaths){
                    ...
                    if($regVms){
                        ...
                        $replicaVmPaths | % {
                            $regParams = @{
                                ResourcePool = [VMware.VimAutomation.ViCore.Types.V1.Inventory.VIContainer]$DestinationCluster
                        ...
                        if($newSourcePg){
                            ...
                            if(!(Compare ($newGroupFull | select * -Exc Name).PSObject.Properties ($srcGroupFull | select * -Exc Name).PSObject.Properties)){
                                ...
                                if($sourceSp){
                                    if($newSrcReplGroup){
                                        if($srcGroupDisks -or $srcGroupVms){
                                            ...
                                            # If there are any (may have been removed/delted), re-apply policy to VMDKs then VMs in original Protection Group. Fails if VMs are done first
                                            ...
                                            if($disksToRefresh){
                                                ...
                                                if((($vmsToRefresh | Get-SpbmEntityConfiguration).ComplianceStatus | select -Unique) -eq 'compliant'){
                                                    "$( (Get-Date).ToString() ) Failed to re-apply $( $sourceSp.Name ) to VMs in $sourcePgName`n" | Write-Warning
                                                } elseif((($vmsToRefresh | Get-SpbmEntityConfiguration).ReplicationGroup.Name | select -Unique) -ne $newSrcReplGroup.Name){
                                                    "$( (Get-Date).ToString() ) Failed to add VMs to $sourcePgName. Retrying ...`n" | Write-Warning
                                                    $vmsToRefresh | Get-SpbmEntityConfiguration | ? {$_.ReplicationGroup.Name -ne $newSrcReplGroup.Name } | % {
                                                        Set-SpbmEntityConfiguration -Configuration $_ -ReplicationGroup $newSrcReplGroup | Out-Null
                                                    }
                                                }
                                ...
                                if((Get-PfaProtectionGroup -Array $sourceFlashArray -Name $sourcePgName).volumes){                                
                                    "$( (Get-Date).ToString() ) Creating new snapshot for new $sourcePgName to be replicated to $( $tgtReplGroup.FaultDomain.Name ) ...`n" | Write-Verbose
                ...
                "$( (Get-Date).ToString() ) Removing unwanted vVols on target array that were also in $( $srcGroup.Name )" | Write-Verbose
                ...
                if($configVvol){
                    ...
                    if($pGroup){                    
                        ...
                        # Exclude vVols from removal that are in $replicaVmPaths (VMs not in source VMs list already excluded from that variable)
                                ...
                                if(!($unregisteredVols | % { Get-PfaVolumeProtectionGroups -Array $targetFlashArray -VolumeName $_ -ErrorAction SilentlyContinue })){
                                    ...
                                    if(!(Compare (Get-PfaVolumes -Array $targetFlashArray).name $unregisteredVols -IncludeEqual | ? SideIndicator -eq '==')){
                                        ...
                                        if($unregisteredVols | % { ($_ -split '/')[0] } | select -Unique | % {
                                            ...
                                            "$( (Get-Date).ToString() ) Unable to remove unwanted vVol groups:`n`n" | Write-Warning
                                            $unregisteredVols | % { ($_ -split '/')[0] } | select -Unique | Write-Host -ForegroundColor Yellow
                                            "" | Write-Host
                                        }
                                    } else {
                                        "$( (Get-Date).ToString() ) Unable to remove unwanted vVols. Check volume groups:`n`n" | Write-Warning
                                        $unregisteredVols | % { ($_ -split '/')[0] } | select -Unique | Write-Host -ForegroundColor Yellow
                                        "" | Write-Host  
                                    }                                                               
                                } else {
                                    "$( (Get-Date).ToString() ) Unable to remove unwanted vVols from Protection Group. Skipping removal of unwanted vVols:`n`n" | Write-Warning
                                    $unregisteredVols | Write-Host -ForegroundColor Yellow
                                    "" | Write-Host
                                }
                            } else {
                                "$( (Get-Date).ToString() ) Unable to disconnect hosts completely from unwanted vVols. Skipping removal of unwanted vVols:`n`n" | Write-Warning
                                $unregisteredVols | Write-Host -ForegroundColor Yellow
                                "" | Write-Host
                            }
                        }
                        ...
                        if(Get-PfaProtectionGroup -Array $targetFlashArray -Name $sourcePgName -ErrorAction SilentlyContinue | ? time_remaining -eq $null){
                            ...
                            if(!(Compare ($autoTargetGroupFull | select * -Exc Name).PSObject.Properties ($targetGroupFull | select * -Exc Name).PSObject.Properties)){
                                ...
                                $prompt = "`nMove replica VM to that group? If answer is no then it will be renamed `"$newDuplicateTargetGroupName`" and the automatically created"
                                $prompt += " reverse replication Protection Group, where the replica VM is currently and that has the same settings as the source Protection"
                            ...
                            if($sp -and $replGroup){
                                ...
                                $replicaVms | Get-HardDisk | % {
                                    ...
                                    if(($_ | Get-SpbmEntityConfiguration).StoragePolicy -ne $sp){
                                        "$( (Get-Date).ToString() ) Failed to assign storage policy $( $sp.Name ) to $( $_.Parent.Name ) $( $_.Name )`n" | Write-Warning 
                                    }
                                }
                                ...
                                $replicaVms | %{
                                    ...
                                    if(($_ | Get-SpbmEntityConfiguration).StoragePolicy -ne $sp){
                                        "$( (Get-Date).ToString() ) Failed to assign storage policy $( $sp.Name ) to $( $_.Name )`n" | Write-Warning 
                                    }
                                }
                            } else {
                    ...
                    "`n$( (Get-Date).ToString() ) Unable to retrieve replica config vVol. Skipping removal of unwanted vVols, Protection Group clean-up and storage policy management ...`n" |
                    Write-Warning
                }
